...
async def current(ctx, arg):
    complete_api_link="https://api.openweathermap.org/data/2.5/weather?q="+ arg +"&appid=6e2d2db27e5a42fa384a698d859fc686"
    ...
    if api_data['cod']=='404':
        ...
        embed=discord.Embed(title=f'Here is {arg}\' weather right now', description='Reminder that it\'s not always accurate, pls no bully me', color=0xce2029)\
            .add_field(name='Weather description:',value=f"{api_data['weather'][0]['description']}", inline=False)\
            .add_field(name='Average temperature', value=f"{int(temperature):,.2f} °C",inline=False)\
            .add_field(name='Humidity',value=f"{api_data['main']['humidity']}%", inline=False)\
            .add_field(name='Wind speed', value=f"{api_data['wind']['speed']} km/h", inline=False)
